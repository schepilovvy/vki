...
export const getDisciplinesApi = async (): Promise<DisciplineInterface[]> => {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API}disciplines`);
...
export const getDisciplinesByGroupApi = async (groupId: number): Promise<DisciplineInterface[]> => {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API}disciplines/group/${groupId}`);
...
export const deleteDisciplineApi = async (disciplineId: number): Promise<number> => {
  ...
  debugger;
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API}disciplines/${disciplineId}`, {
      method: 'DELETE',
    });
